...
export default function CaseDetail() {
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center h-screen">
        <div className="text-xl font-semibold text-red-600 mb-4">Error Loading Case</div>
  ...
  const getNestedValue = (obj: any, path: string, fallback: any = "N/A") => {
    ...
    return keys.reduce((acc, key) => 
      (acc && acc[key] !== undefined) ? acc[key] : null, obj) || fallback;
  };

  const getTimelineEvents = () => {
    return getNestedValue(caseData, 'Public.timeline_of_proceedings', []).map((item: any, idx: number) => ({
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold">
            Case #{caseData.CaseID || caseData._id.substring(0, 8)}
          </h1>
          <p className="text-muted-foreground">
            {getNestedValue(caseData, 'Public.case_type', 'Case')} • 
            Filed: {new Date(caseData.createdAt || new Date()).toLocaleDateString()}
          </p>
        </div>
        <Badge className="px-3 py-1 text-sm">
          {getNestedValue(caseData, 'Public.case_status', '').split('.')[0] || "Active"}
        </Badge>
      </div>
        ...
        <div className="grid gap-6 md:grid-cols-3">
          <Card className="md:col-span-2">
            ...
            <CardContent className="space-y-4">
              <p>{getNestedValue(caseData, 'Public.case_summary', 'No case summary available.')}</p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <p>{getNestedValue(caseData, 'Public.court_details.name', 'Not specified')}</p>
                </div>
                <div>
                  <h4 className="font-medium text-muted-foreground">Presiding Judge</h4>
                  <p>{getNestedValue(caseData, 'Public.court_details.presiding_judge', 'Not assigned')}</p>
                </div>
              </div>
              
              <h3 className="font-semibold text-lg mt-4">Parties Involved</h3>
              <div>
                <h4 className="font-medium text-muted-foreground">Prosecution/Plaintiff</h4>
                <ul className="list-disc list-inside pl-2">
                  {getNestedValue(caseData, 'Public.parties.prosecution', []).length > 0 ? 
                    getNestedValue(caseData, 'Public.parties.prosecution', []).map((party: string, idx: number) => (
                      <li key={idx}>{party}</li>
                    )) : 
                ...
                <h4 className="font-medium text-muted-foreground mt-2">Defendant</h4>
                <ul className="list-disc list-inside pl-2">
                  {getNestedValue(caseData, 'Public.parties.defendant', []).length > 0 ? 
                    getNestedValue(caseData, 'Public.parties.defendant', []).map((party: string, idx: number) => (
                      <li key={idx}>{party}</li>
                    )) : 
          ...
          <div className="space-y-6">
            <Card>
              ...
              <CardContent className="space-y-3">
                <div>
                  <h4 className="font-medium text-muted-foreground">Case Type</h4>
                  <p>{getNestedValue(caseData, 'Public.case_type', 'Not specified')}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p>{new Date(caseData.createdAt || new Date()).toLocaleDateString()}</p>
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                {getTimelineEvents().length > 0 ? (
                  <div>
                    <p className="font-medium">{getTimelineEvents()[getTimelineEvents().length - 1]?.event}</p>
                    <p className="text-sm text-muted-foreground mt-1">
                      {getTimelineEvents()[getTimelineEvents().length - 1]?.date}
                    </p>
                  </div>
                ) : (
      ...
      {activeTab === 'evidence' && (
        <div className="space-y-6">
          <Card>
            ...
            <CardContent>
              <p className="mb-4">{getNestedValue(caseData, 'Private.evidence_summary', 'No evidence summary available.')}</p>
            </CardContent>
          </Card>
          ...
          {Object.entries(caseData.Evidence || {}).map(([category, items]) => {
            ...
            return (
              <Card key={category}>
                <CardHeader>
                  <CardTitle>{category.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}</CardTitle>
                </CardHeader>
                <CardContent>
                  <ul className="space-y-2">
                    {items.map((item: string, idx: number) => (
                      <li key={idx} className="p-3 border rounded-md bg-accent/5">
                        {item}
                      </li>
                    ))}
                  </ul>
          ...
          <CardContent>
            <div className="space-y-6">
              {getTimelineEvents().length > 0 ? (
                ...
                <p className="text-center py-6 text-muted-foreground">No timeline events available</p>
              )}
            </div>
          </CardContent>
        </Card>
